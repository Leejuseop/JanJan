...
exports.onSessionClosed = exports.onCvFrameCreate = exports.onDetectionUpdate = void 0;
...
exports.onDetectionUpdate = functions.firestore
...
exports.onCvFrameCreate = functions.firestore
...
function normalizeDrinkTypeOrNull(value) {
...
function boundedNumberValue(value, min, max) {
    const parsed = numberValue(value);
    if (parsed == null) {
        return null;
    }
...
function dateKeyFromKstMillis(millis) {
    return new Date(millis + KST_OFFSET_MS).toISOString().slice(0, 10);
}
async function aggregateClosedSession(sessionId, sessionData, endedAtMs) {
    ...
    for (const participant of participants.docs) {
        await aggregateParticipantDrinkStats(sessionId, dateKey, storeId, storeName, endedAtMs, participant);
    }
}
async function aggregateParticipantDrinkStats(sessionId, dateKey, storeId, storeName, endedAtMs, participant) {
...
async function handleColorGlassProximity(sessionId, eventId, eventData, durationMs) {
    ...
    await mapColorToPhysicalGlass(sessionId, eventId, screenColorHex, physicalGlassId, drinkType);
}
async function mapColorToPhysicalGlass(sessionId, eventId, screenColorHex, physicalGlassId, drinkType) {
    ...
    duplicateGlassMappings.docs.forEach((doc) => {
...
async function handlePourDetection(sessionId, eventId, eventData, durationMs) {
    ...
    await incrementDrinkForGlass(sessionId, eventId, glassId, durationMs, normalizeDrinkTypeOrNull(eventData.drinkType));
}
async function incrementDrinkForGlass(sessionId, eventId, glassId, durationMs, expectedDrinkType) {
...
async function handleCvFrame(sessionId, frameId, frameData) {
    ...
    const colorMappingThresholdMs = boundedNumberValue(frameData.colorMappingThresholdMs, 500, 10000) ?? COLOR_GLASS_MAPPING_THRESHOLD_MS;
    const pourThresholdMs = boundedNumberValue(frameData.pourThresholdMs, 500, 10000) ?? CV_POUR_THRESHOLD_MS;
    if (detections.length === 0) {
        return;
    }
    const colorScreens = detections.filter((detection) => YOLO_COLOR_SCREEN_TYPES.has(detection.objectType) && detection.screenColorHex != null);
    const glasses = detections.filter((detection) => detection.drinkType != null);
    const bottles = detections.filter((detection) => YOLO_SOJU_BOTTLE_TYPES.has(detection.objectType)
        || YOLO_BEER_BOTTLE_TYPES.has(detection.objectType));
    const activeColorPairIds = new Set();
    const activePourPairIds = new Set();
    for (const screen of colorScreens) {
        for (const glass of glasses) {
            ...
            const state = await updateCvPairState(sessionId, pairId, "colorGlass", capturedAtMs, colorMappingThresholdMs, "mappedAt", {
                cameraId,
                frameId,
            ...
            if (state.shouldFire) {
                await mapColorToPhysicalGlass(sessionId, `cv:${frameId}:${pairId}`, screen.screenColorHex, physicalGlassId, glass.drinkType);
            }
        }
    }
    for (const glass of glasses) {
        ...
        for (const bottle of bottles) {
            ...
            const state = await updateCvPairState(sessionId, pairId, "pour", capturedAtMs, pourThresholdMs, "countedAt", {
                cameraId,
                frameId,
                glassTrackId: glass.trackId,
            ...
            if (state.shouldFire) {
                await incrementDrinkForGlass(sessionId, `cv:${frameId}:${pairId}`, physicalGlassId, state.durationMs, glass.drinkType);
            }
        }
    }
...
function parseCvDetections(frameData, cameraId, frameWidth, frameHeight) {
    ...
    return rawDetections.flatMap((raw, index) => {
        ...
        const data = raw;
        const objectType = normalizeObjectType(normalizeString(data.objectType)
        ...
        const trackId = sanitizeDocId(normalizeString(data.trackId)
            ?? normalizeString(data.objectId)
            ?? normalizeString(data.id)
            ?? `${objectType}_${index}`);
        const screenColorHex = normalizeHex(data.screenColorHex ?? data.colorHex ?? data.assignedColorHex) ?? undefined;
...
function resolveDetectionCenter(data, frameWidth, frameHeight) {
...
function normalizePoint(x, y, frameWidth, frameHeight) {
...
function drinkTypeForGlassObjectType(objectType) {
...
function drinkTypeForBottle(detection) {
...
function distanceBetween(first, second, frameData) {
...
async function updateCvPairState(sessionId, pairId, pairType, seenAtMs, thresholdMs, terminalField, payload) {
...
async function markInactiveMissingPairs(sessionId, cameraId, pairType, activePairIds, seenAtMs) {
